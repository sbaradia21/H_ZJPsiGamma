...
def dumpInJobCard(jfile, era, commonInfoList, skimInfoList, mvaInfoList, histDir, key,xsec, dataset, files, ismc, isdata, issignal):
    if ismc:
        jfile.write('START\n'+'era '+str(era)+'\n'+'dataType mc\n')
    if issignal:
        jfile.write('START\n'+'era '+str(era)+'\n'+'dataType mc#signal\n')
...
#    jfile.write('evtWtSum '+evtWtSum+'\n')
    ...
    jfile.write('########### xsec, hist ###########\n')
    if not isdata:
        jfile.write('xsec '+str(xsec)+'\n')
        ...
        if valDict == None or len(valDict) == 0:
            ...
            for key, val in valDict.items():
                ...
                with open(os.path.join(jobDir,jobFile), 'w') as jfile:
                    dumpInJobCard(jfile, era, commonInfoList, skimInfoList, mvaInfoList, histDir, key,xsec,dataset, files, ismc, isdata, issignal)
                ...
                with alive_bar(len(infileListPerJob),title='Preparing Condor Scripts', enrich_print=True, bar='filling') as bar:
                    for i, filesList in enumerate(infileListPerJob):
                        ...
                        with open(jobkey, 'w') as tmpl:
                            tmplKey = key+'_'+str(i)
                            dumpInJobCard(tmpl, era, commonInfoList, skimInfoList, mvaInfoList, batchHistDir, tmplKey, xsec,dataset, filesList, ismc, isdata, issignal)
